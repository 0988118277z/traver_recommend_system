        ...
        if 'name' in request.session:
            ...
            if form.is_valid():
                context_dict = {'userreuse': 0, 'password': 0, 'emailreuse': 0, 'phonereuse': 0,
                                ...
                                'email': form.cleaned_data["email"],
                                'phone': form.cleaned_data["phone"]}
                password = Webusers.objects.get(account=account).password
                email = Webusers.objects.get(account=account).email
                username = Webusers.objects.get(account=account).username
                phone = Webusers.objects.get(account=account).username
                ...
                if phone != form.cleaned_data["phone"]:
                    context_dict['phone'] = ''

                if username != form.cleaned_data["username"]:
                ...
                if 1 in context_dict.values():
                    print(context_dict)
                    return render(request, 'account/fix.html', context=context_dict)
            ...
            # try:
            print(f'=={form.cleaned_data["email"]}==')
            user_id = Webusers.objects.get(
                email=form.cleaned_data["email"]).user_id
            if user_id:
                verify_token = hashlib.sha256(
                    str(uuid.uuid4()).encode('utf-8')).hexdigest()  # 製作token

                current_time = datetime.now()  # 取得當前時間
                new_time = current_time + \
                    timedelta(hours=8, minutes=10)  # 增加10分鐘
                formatted_time = new_time.strftime(
                    "%Y-%m-%d %H:%M:%S")  # 格式化時間

                RegisterVerify(
                    account_id_id=user_id, token_expiration=formatted_time, verify_token=verify_token).save()

                print('token ok')
                mailcontent = f'請點選下方連結啟用帳號' + '\n' + \
                    f'https://travel.yiiii.org/account/fix/?key={verify_token}'
                send_mail('啟用帳號', mailcontent, "dspring1995@gmail.com",
                          [form.cleaned_data["email"]])
                print('倌人 ~~ mail success')
